...
area_colors <- c("#e88b1a", "#e09a43", "#dea968", "#dbba91", # FFA2
                 "#d12e11", "#cf503a", "#cf7767", "#cc968d", # FFA1
...
df_uni_tb <- left_join(df_uni_tb, df_uni_tb_sig) %>% 
  mutate(p.value = if_else(is.na(p.value), 1, p.value),
         Stimuli = as_factor(Stimuli),
         Layout = factor(Layout, levels = c("top", "bottom", "partA", "partB"))) 
...
plot_decoding <- function(df, exp, stim, is_plot=FALSE, str_plot=""){
  ...
  if (is_plot){
    plot_fn <- sprintf('plot_decode_%s_%s%s.pdf', toupper(exp), str_remove_all(stim, '\n'), str_plot)
    print(plot_fn)
...
```{r}
df_area_uni_plot <- df_area_uni %>% 
  mutate(Area = fct_rev(Area),
         Stimuli = FaceWord,
         Hemisphere = if_else(substr(ROI, 1, 1)=="l", "left", "right"),
         HROI = toupper(ROI),
         ROI = substr(ROI, 2, 5),
         ROI = factor(ROI, levels = roi_order_pub),
         HROI = factor(HROI, levels = hroi_order_pub),
...
```{r fig.width=6, fig.asp=0.5}
plot_uni_area <- function(df, exp, stim, contr, is_plot=FALSE){
  
  plot_this <- ggplot(filter(df, Exp==exp, Stimuli==stim, contrast==contr), 
                      aes(x = HROI, y = estimate, fill = interaction(ROI, Area, sep="-", lex.order = T))) +
    geom_col(position = position_dodge()) +
    geom_errorbar(aes(ymin=lower.CL, ymax=upper.CL), size = 0.25,
                  position=position_dodge2(width=0.5, padding=0.5)) +
    facet_grid(cols = vars(Hemisphere), scales = "free_x", space = "free_x", 
               switch = "x") +
  ...
  if (is_plot){
    plot_fn <- sprintf('plot_uni_area_%s_%s_%s.pdf', toupper(exp), stim, contr)
    ggsave(plot_fn, plot_this, width = 11, height = 11*.4)
  }
...
plot_uni_area(df_area_uni_plot, "E1", "faces", "intact - exchange")
# plot_uni_area(df_area_uni_plot, "E1", "faces", "intact - exchange", 1)
```

```{r fig.width=6, fig.asp=0.4}
plot_uni_area(df_area_uni_plot, "E1", "words", "intact - exchange")
# plot_uni_area(df_area_uni_plot, "E1", "words", "intact - exchange", 1)
```

```{r fig.width=6, fig.asp=0.4}
plot_uni_area(df_area_uni_plot, "E2", "English", "intact - exchange")
# plot_uni_area(df_area_uni_plot, "E2", "English", "intact - exchange", 1)
```

```{r fig.width=6, fig.asp=0.4}
plot_uni_area(df_area_uni_plot, "E2", "Chinese", "intact - exchange")
# plot_uni_area(df_area_uni_plot, "E2", "Chinese", "intact - exchange", 1)
```


### top - bottom
```{r fig.width=6, fig.asp=0.4}
plot_uni_area(df_area_uni_plot, "E1", "faces", "top - bottom")
# plot_uni_area(df_area_uni_plot, "E1", "faces", "top - bottom", 1)
```

```{r fig.width=6, fig.asp=0.4}
plot_uni_area(df_area_uni_plot, "E1", "words", "top - bottom")
# plot_uni_area(df_area_uni_plot, "E1", "words", "top - bottom", 1)
```

```{r fig.width=6, fig.asp=0.4}
plot_uni_area(df_area_uni_plot, "E2", "English", "top - bottom")
# plot_uni_area(df_area_uni_plot, "E2", "English", "top - bottom", 1)
```

```{r fig.width=6, fig.asp=0.4}
plot_uni_area(df_area_uni_plot, "E2", "Chinese", "top - bottom")
# plot_uni_area(df_area_uni_plot, "E2", "Chinese", "top - bottom", 1)
```

## Decoding

```{r}
df_area_decode_plot <- df_area_decoding %>% 
  mutate(Hemisphere = if_else(substr(ROI, 1, 1)=="l", "left", "right"),
         HROI = toupper(ROI),
         ROI = substr(ROI, 2, 5),
         ROI = factor(ROI, levels = roi_order_pub),
         HROI = factor(HROI, levels = hroi_order_pub),
...
```{r fig.width=6, fig.asp=0.5}
plot_decode_area <- function(df, exp, pair, is_plot=FALSE, str_plot=""){
  
  plot_this <- ggplot(filter(df, Exp==exp, ClassifyPair==pair), 
                      aes(x = HROI, y = mean, fill = interaction(ROI, Area, sep="-", lex.order = T))) +
    geom_col(position = position_dodge()) +
    geom_errorbar(aes(ymin=lower.CL, ymax=upper.CL), size = 0.25,
                  position=position_dodge2(width=0.5, padding=0.5)) +
    facet_grid(cols = vars(Hemisphere), scales = "free_x", space = "free_x", 
  ...
  if (is_plot){
    plot_fn <- sprintf('plot_decode_area_%s_%s%s.pdf', toupper(exp), pair, str_plot)
    ggsave(plot_fn, plot_this, width = 11, height = 11*.27)
  }
...
plot_decode_area(df_area_decode_plot, "E1", "face_intact-word_intact")
# plot_decode_area(df_area_decode_plot, "E1", "face_intact-word_intact", 1)
```

```{r fig.width=6, fig.asp=0.3}
plot_decode_area(df_area_decode_plot, "E1", "face_intact-face_exchange")
# plot_decode_area(df_area_decode_plot, "E1", "face_intact-face_exchange", 1)
```

```{r fig.width=6, fig.asp=0.3}
plot_decode_area(df_area_decode_plot, "E1", "word_intact-word_exchange")
# plot_decode_area(df_area_decode_plot, "E1", "word_intact-word_exchange", 1)
```

```{r fig.width=6, fig.asp=0.3}
plot_decode_area(df_area_decode_plot, "E1", "face_top-face_bottom")
# plot_decode_area(df_area_decode_plot, "E1", "face_top-face_bottom", 1)
```

```{r fig.width=6, fig.asp=0.3}
plot_decode_area(df_area_decode_plot, "E1", "word_top-word_bottom")
# plot_decode_area(df_area_decode_plot, "E1", "word_top-word_bottom", 1)
```

```{r fig.width=6, fig.asp=0.3}
plot_decode_area(df_area_decode_plot, "E2", "English_intact-Chinese_intact")
# plot_decode_area(df_area_decode_plot, "E2", "English_intact-Chinese_intact", 1)
```

```{r fig.width=6, fig.asp=0.3}
plot_decode_area(df_area_decode_plot, "E2", "English_intact-English_exchange")
# plot_decode_area(df_area_decode_plot, "E2", "English_intact-English_exchange", 1)
```

```{r fig.width=6, fig.asp=0.3}
plot_decode_area(df_area_decode_plot, "E2", "Chinese_intact-Chinese_exchange")
# plot_decode_area(df_area_decode_plot, "E2", "Chinese_intact-Chinese_exchange", 1)
```

```{r fig.width=6, fig.asp=0.3}
plot_decode_area(df_area_decode_plot, "E2", "English_top-English_bottom")
# plot_decode_area(df_area_decode_plot, "E2", "English_top-English_bottom", 1)
```

```{r fig.width=6, fig.asp=0.3}
plot_decode_area(df_area_decode_plot, "E2", "Chinese_top-Chinese_bottom")
# plot_decode_area(df_area_decode_plot, "E2", "Chinese_top-Chinese_bottom", 1)
```
...
```{r fig.width=6, fig.asp=0.5}
plot_uni_area_roi <- function(df, roi, is_plot=FALSE){
  
  # whichROI <- which(substring(roi, 2)==levels(df$ROI))
  # area_color <- area_colors[seq(whichROI, whichROI+3)]
  
  area_color <- c("#bdc9e1", "#74a9cf", "#2b8cbe", "#045a8d") # blue
  df <- mutate(df, Area = fct_rev(Area))
  
  plot_this <- ggplot(filter(df, HROI==roi), 
                      aes(x = HROI, y = estimate, fill = Area)) +
    geom_col(position = position_dodge()) +
    geom_errorbar(aes(ymin=lower.CL, ymax=upper.CL), size = 0.25,
                  position=position_dodge2(width=0.5, padding=0.5)) +
    facet_grid(cols = vars(Exp, Stimuli), rows = vars(contrast), scales = "free_x", space = "free_x", 
               switch = "x") +
    coord_cartesian(ylim = c(-.5, .45)) + 
    scale_y_continuous(expand = expansion(mult = c(0, .2))) +
    labs(y = "Beta values") +  # set the names for main, x and y axises
  ...
  if (is_plot){
    plot_fn <- sprintf('plot_uni_area_%s.pdf', roi)
    ggsave(plot_fn, plot_this, width = 8, height = 8)
  }
...
```{r}
plot_uni_area_roi(df_area_uni_plot, "LFFA1", 1)
```

```{r}
plot_uni_area_roi(df_area_uni_plot, "LFFA2", 1)
```

```{r}
plot_uni_area_roi(df_area_uni_plot, "LVWFA", 1)
```

```{r}
plot_uni_area_roi(df_area_uni_plot, "LLO", 1)
```

```{r}
plot_uni_area_roi(df_area_uni_plot, "RFFA2", 1)
```

```{r}
plot_uni_area_roi(df_area_uni_plot, "RFFA1", 1)
```

```{r}
plot_uni_area_roi(df_area_uni_plot, "RLO", 1)
```

### Decoding
```{r}
df_area_decode_roi <- df_area_decode_plot %>% 
  mutate(ClassifyPair = str_replace(ClassifyPair, "-", "-\n"),
         ClassifyPair = factor(ClassifyPair, 
                               c("face_intact-\nword_intact", 
                                 "face_intact-\nface_exchange",
                                 "word_intact-\nword_exchange",
                                 "face_top-\nface_bottom",
...
```{r fig.width=6, fig.asp=0.5}
plot_decode_area_ROI <- function(df, roi, is_plot=FALSE){
  ...
  plot_this <- ggplot(filter(df, HROI==roi), 
                      aes(x = HROI, y = mean, fill = Area)) +
    geom_col(position = position_dodge()) +
    geom_errorbar(aes(ymin=lower.CL, ymax=upper.CL), size = 0.25,
                  position=position_dodge2(width=0.5, padding=0.5)) +
    facet_wrap(vars(ClassifyPair, Exp), scales = "free_x", # space = "free_x", 
  ...
  if (is_plot){
    plot_fn <- sprintf('plot_decode_area_%s.pdf', roi)
    ggsave(plot_fn, plot_this, width = 10, height = 10)
  }
...
```{r fig.width=6, fig.asp=0.5}
plot_decode_area_ROI(df_area_decode_roi, "LFFA1", 1)
# plot_decode_area_ROI(df_area_decode_roi, "LFFA1", 1)
```

```{r fig.width=6, fig.asp=0.5}
plot_decode_area_ROI(df_area_decode_roi, "LFFA2", 1)
```

```{r fig.width=6, fig.asp=0.5}
plot_decode_area_ROI(df_area_decode_roi, "LVWFA", 1)
```

```{r fig.width=6, fig.asp=0.5}
plot_decode_area_ROI(df_area_decode_roi, "LLO", 1)
```

```{r fig.width=6, fig.asp=0.5}
plot_decode_area_ROI(df_area_decode_roi, "RFFA1", 1)
```

```{r fig.width=6, fig.asp=0.5}
plot_decode_area_ROI(df_area_decode_roi, "RFFA2", 1)
```

```{r fig.width=6, fig.asp=0.5}
plot_decode_area_ROI(df_area_decode_roi, "RLO", 1)
```
