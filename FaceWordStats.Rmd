...
desc_mvpa_acc <- {
  df_mvpa_acc %>% 
    group_by(ExpCode, ROI, ClassifyPair) %>% 
...
### Label:left FFA
```{r the label used for left FFA E1}
...
```{r only keep data for left FFA E1}
# only keep data for this label
df_univar_E1_lFFA <- filter(df_univar_E1, ROI %in% label_lFFA_E1)
df_univar_agg_E1_lFFA <- filter(df_univar_agg_E1, ROI %in% label_lFFA_E1)
df_mvpa_E1_lFFA <- filter(df_mvpa_E1, ROI %in% label_lFFA_E1)
df_mvpa_acc_E1_lFFA <- filter(df_mvpa_acc_E1, ROI %in% label_lFFA_E1)
desc_mvpa_acc_E1_lFFA <- filter(desc_mvpa_acc_E1, ROI %in% label_lFFA_E1)

# nlevels(df_univar_E1_lFFA$SubjCode)
```
The number of subjects included in this analysis is `r nlevels(df_univar_E1_lFFA$SubjCode)`: `r unique(as.character(df_univar_E1_lFFA$SubjCode))`.

#### Univariate analyses
##### rm-ANOVA 
```{r anova for univariate E1 lFFA}
anova_E1_lFFA <- aov_4(meanResp ~ FaceWord * Layout + (1 + FaceWord * Layout | SubjCode), 
                      data = df_univar_agg_E1_lFFA)
...
##### Plot
```{r plots for functional scans E1 (anova) lFFA}
plot_uni_E1_lFFA <- {
  ggplot(data = as.data.frame(emm_anova_E1_lFFA), aes(y = emmean, x = FaceWord, fill = Layout)) + # set the data, varialbes for x and y axises
    geom_col(position = "dodge", color = "black", alpha = .7) +  # position of columns and countour of columns
    geom_errorbar(mapping = aes(ymin = lower.CL, ymax = upper.CL), linetype = 1,  # set the error bar
                  show.legend = FALSE, width = 0.25, alpha = .5,
                  position = position_dodge(width=0.9)) +
...
##### Plot
```{r plots for functional scans mvpa E1 (anova) lFFA}

plot_mvpa_E1_lFFA <- {
  ggplot(data = desc_mvpa_acc_E1_lFFA, aes(y = emmean, x = ClassifyPair)) + # set the data, varialbes for x and y axises
    geom_col(position = "dodge", color = "black", alpha = .7) +  # position of columns and countour of columns
    geom_errorbar(mapping = aes(ymin = lower.CL, ymax = upper.CL), linetype = 1,  # set the error bar
                  show.legend = FALSE, width = 0.25, alpha = .5,
                  position = position_dodge(width=0.9)) +
...
### Label: right FFA
```{r the label used for right FFA E1}
...
```{r only keep data for right FFA E1}
# only keep data for this label
df_univar_E1_rFFA <- filter(df_univar_E1, ROI %in% label_rFFA_E1)
df_univar_agg_E1_rFFA <- filter(df_univar_agg_E1, ROI %in% label_rFFA_E1)
df_mvpa_E1_rFFA <- filter(df_mvpa_E1, ROI %in% label_rFFA_E1)
df_mvpa_acc_E1_rFFA <- filter(df_mvpa_acc_E1, ROI %in% label_rFFA_E1)
desc_mvpa_acc_E1_rFFA <- filter(desc_mvpa_acc_E1, ROI %in% label_rFFA_E1)

# nlevels(df_univar_E1_rFFA$SubjCode)
```
The number of subjects included in this analysis is `r nlevels(df_univar_E1_rFFA$SubjCode)`: `r unique(as.character(df_univar_E1_rFFA$SubjCode))`.

#### Univariate analyses
##### rm-ANOVA 
```{r anova for univariate E1 rFFA}
anova_E1_rFFA <- aov_4(meanResp ~ FaceWord * Layout + (1 + FaceWord * Layout | SubjCode), 
                      data = df_univar_agg_E1_rFFA)
