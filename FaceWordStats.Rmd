...
## How the labels are created/defined for each ROI?
1. Identify the vertex whose beta value is larger than the surrounding vertices (i.e., the global maxima) for each ROI based on the reference coordinates in previous literature.
2. Dilate the region centering at the global maxima until it reaches the pre-defined size (100mm^2), during which the vertices are masked by a pre-defined label at the threshold of p < .05. In other words, the p-values for all vertices in the labels are smaller than .05 (uncorrected).
...
```{r E2}
pair_order_E2 <- c("Chinese_intact-English_intact",
                   "Chinese_intact-Chinese_exchange",
                   "Chinese_part1-Chinese_part2",
                   "English_intact-English_exchange",
                   "English_part1-English_part2")
```
