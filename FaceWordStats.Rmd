...
# Introduction
## Usage of this file
This file serves to be a supplementary document that describes all the statistics results performed for this project. It may help to test some new hypotheses, which are not included in the corresponding slides. [Please refers to the slides for the research questions of this project.] 
...
label_ROI
```
The above table displays the label used for each ROI and each Experiment. (VWFA is only found on the left hemisphere.)
...
```
The ROI (label) used for this analysis is `r unique(df_loc_E1$Label)`.
...
```
The ROI (label) used for this analysis is `r unique(df_loc_E2$Label)`.
