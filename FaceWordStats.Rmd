...
plot_intact_all <- ggplot(df_intact, aes(y = mean, x = Hemisphere)) +
  ...
  labs(title = "Intact stimuli", x = "Hemipshere", y = "Accuracy") +  # set the names for main, x and y axises
...
# ggsave('plot_decode_all_intact.png', plot_intact_all, width = 8, height = 5)
...
plot_inex_all <- ggplot(df_inex, aes(y = mean, x = Hemisphere)) +
  geom_col(position = "dodge", width = .5, fill = "#CDCDC8") +
  facet_grid(Exp + Stimuli ~ ROI) +
  geom_errorbar(mapping = aes(ymin = lower.CL, ymax = upper.CL), linetype = 1,  # set the error bar
                show.legend = FALSE, width = 0.25, alpha = .5,
                position = position_dodge(width=0.9)) +
  geom_hline(yintercept = 0.5, linetype = 5, alpha = 0.5) +  # add the line for 0.5 and 1 (y)
  ...
  scale_y_continuous(expand= c(0, 0), breaks = seq(0, 1, .25)) +  # remove the space between columns and x axis
  coord_cartesian(ylim = c(0, 1.1)) +
  labs(title = "Intact vs. Exchange", x = "Hemipshere", y = "Accuracy") +  # set the names for main, x and y axises
  # geom_text(aes(label = sig_ast(p)), size = 7, nudge_y = 0.15) + # add starts to the significant columns
  geom_text(aes(label = round(mean, 2)), size = 4, nudge_y = 0.2) +
  # geom_text(data = dat_text, mapping = aes(x = c(.8, 1.2), y = y, label = label), size = 6.5, fontface = "bold") +
  theme_bw() +
...
# ggsave('plot_decode_all_inex.png', plot_inex_all, width = 6, height = 5)
...
plot_topbottom_all <- ggplot(df_parts, aes(y = mean, x = Hemisphere)) +
  geom_col(position = "dodge", width = .5, fill = "#CDCDC8") +
  facet_grid(Exp + Stimuli ~ ROI) +
  geom_errorbar(mapping = aes(ymin = lower.CL, ymax = upper.CL), linetype = 1,  # set the error bar
                show.legend = FALSE, width = 0.25, alpha = .5,
  ...
  scale_y_continuous(expand= c(0, 0), breaks = seq(0, 1, .25)) +  # remove the space between columns and x axis
  coord_cartesian(ylim = c(0, 1.1)) +
  labs(title = "Top vs. bottom; left vs. right", x = "Hemipshere", y = "Accuracy") +  # set the names for main, x and y axises
  geom_text(aes(label = sig_ast(p)), size = 7, nudge_y = 0.15) + # add starts to the significant columns
  # geom_text(aes(label = round(mean, 2)), size = 4, nudge_y = 0.2) +
  # geom_text(data = dat_text, mapping = aes(x = c(.8, 1.2), y = y, label = label), size = 6.5, fontface = "bold") +
  theme_bw() +
...
# ggsave('plot_decode_all_topbottom.png', plot_topbottom_all, width = 6, height = 5)
