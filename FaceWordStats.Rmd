...
ROI <- c("FFA", "FFA1", "FFA2", "VWFA")
left_E1 <- c(label_lFFA_E1, label_lFFA1_E1, label_lFFA2_E1, label_VWFA_E1)
right_E1 <- c(label_rFFA_E1, label_rFFA1_E1, label_rFFA2_E1, 'NA')
left_E2 <- c(label_lFFA_E2, label_lFFA1_E2, label_lFFA2_E2, label_VWFA_E2)
right_E2 <- c(label_rFFA_E2, label_rFFA1_E2, label_rFFA2_E2, 'NA')
...
desc_mvpa_acc %>%
  select(ExpCode, ROI, Count) %>% 
  distinct() %>% 
  filter(ROI %in% c(left_E1, right_E1, left_E2, right_E2)) %>% 
  mutate(Hemi = substr(ROI, 5,6),
         Hemisphere = if_else(Hemi == "lh", "left", if_else(Hemi == "rh", "right", "NA")),
         Col_name = paste(Hemisphere, paste0("E", ExpCode), sep = "_"),
         ROI = if_else(grepl("ffa1", ROI), "FFA1", if_else(grepl("ffa2", ROI), "FFA2", if_else(grepl("w-vs-o", ROI), "VWFA", "FFA")))) %>% 
...
# df_mvpa %>%
#   filter(nVertices < nVtx_min) %>% # remove the label whose vertex number is smaller than nVtx_min
#   select(ExpCode, ROI, SubjCode) %>%
#   distinct() %>%
#   group_by(ExpCode, ROI) %>%
#   summarize(Count = n()) %>%
#   ungroup() %>% 
#   spread(ROI, Count)
